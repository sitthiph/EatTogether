...
/**
 * Handles database operations for the MyOrder.
 */
class MyOrderRepository {
  ...
  async create(data, options) {
    ...
    await FirebaseRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(
          `${new MyOrder().collectionName}/${record.id}`,
        ),
      record,
      options,
    );
  ...
  async destroy(id, options) {
    await FirebaseRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(`${new MyOrder().collectionName}/${id}`),
      null,
      options,
    );
  ...
  /**
   * Counts the number of MyOrders based on the filter.
   *
   * @param {Object} filter
   */
  async count(filter) {
    let chain = admin
      .firestore()
      .collection(new MyOrder().collectionName);
  ...
  /**
   * Finds the MyOrder and its relations.
   *
   * @param {string} id
   */
  async findById(id) {
  ...
  /**
   * Finds the MyOrders based on the query.
  ...
  async findAndCountAll(
    ...
    if (filter) {
      ...
      if (filter.restaurantName) {
        query.appendIlike('restaurantName', filter.restaurantName);
      }
  ...
  /**
   * Lists the MyOrders to populate the autocomplete.
  ...
  async _createAuditLog(action, id, data, options) {
    await AuditLogRepository.log(
      {
        entityName: new MyOrder().modelName,
        entityId: id,
        action,
        values: data,
      },
